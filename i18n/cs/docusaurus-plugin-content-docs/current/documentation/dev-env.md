...
- Abyste mohli provolávat testovací rozhraní (totéž pak platí i pro produkci), je potřeba nejprve získat přístupové údaje *username + password*, díky kterým pak budete moci získat přístupový token (*access token*), bez nějž není možné jednotlivé endpointy rozhraní provolávat (více detailů v [TD](https://csoneclicknew.docs.apiary.io/#reference/security/login-partner)).
...
- Zda je testovací prostředí funkční (a není na něm např. technická odstávka) lze zjistit pomocí health check endpointu (viz [TD](https://csoneclicknew.docs.apiary.io/#reference/health-check/api-health-check/api-health-chceck))
...
### Dostupné varianty entity konzumenta API na testovacím prostředí:
 ...
 A) Prodej **bez akontace** (ID prodejny **024242**)
  ...
  > ***apiKey*** pro kalkulačku splátek ve variantě [javascriptový widget](https://github.com/homecreditcz/widget-calculator "Kalkulačka splátek - varianta javascriptový widget")
  > - CZ: `calculator_test_key`
  > - SK: `calculator_test_key`

  > tajný klíč pro hashování [zpětné komunikace na e-shop](https://github.com/homecreditcz/oneclick-api/wiki/Zabezpe%C4%8Den%C3%AD-zp%C4%9Btn%C3%A9-komunikace-partnersk%C3%A9mu-e%E2%80%90shopu "Zabezpečení zpětné komunikace na e-shop"):
  > - CZ: `!;8ez62oe{*,_`
  > - SK: `%j:o)t:y/8)`
...
Zlatou střední cestou je řešení, kdy vlastní backend e-shopu využívá javascriptový widget od Home Creditu, který obstarává frontendovou část, výpočet vhodných nabídek a na základě výsledku připravuje podklady backendu e-shopu pro provolání endpointu na vytvoření žádosti o úvěr.
   ...
   * Testovací hodnoty jsou popsány v [návodu na instalaci widgetu](https://github.com/homecreditcz/widget-calculator/blob/master/docs/install.md "Instalace javascriptového widgetu - kalkulačky splátek")
...
o   Úplné vzorové requesty, které lze právě pomocí aplikace Postman vyzkoušet, naleznete na konci tohoto dokumentu (případně v [TD](https://csoneclicknewfuture.docs.apiary.io) – u těch je ale potřeba použít relevantní data)

o   Podkladová data pro vzorové requesty lze společně s jednoduchým návodem nalézt také [zde](https://github.com/homecreditcz/oneclick-api/tree/master/create-application-test "Create application - vzorové requesty")
